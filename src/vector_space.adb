...
package body Vector_Space is
   ...
   -- Vector --
   ------------
   function Vector( X,Y,Z : in Value_Type ) return Vector_Type is
      Result : Vector_Type := ( X,Y, Z );
   begin
      return Result;
   end ;

   function Component( X : in Vector_type; I : in Index_Type ) return Value_Type is
   begin
      return X(i);
   end Component;
